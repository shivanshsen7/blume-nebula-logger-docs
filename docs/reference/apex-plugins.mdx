...
```mermaid
flowchart TD
    subgraph Pipeline["Logging / Purge Pipeline"]
        SAVE["Log entry saved"] --> TRIG["Triggerable: execute(LoggerPlugin_t, LoggerTriggerableContext)"]
        RUN["LogBatchPurger batch run"] --> BATCH["Batchable: start / execute / finish(LoggerPlugin_t, LoggerBatchableContext)"]
    end
    TRIG --> ARCHIVE["LogEntryArchivePlugin"]
    TRIG --> RETENTION["LogRetentionRulesPlugin"]
    TRIG --> SLACK["SlackLoggerPlugin"]
    BATCH --> ARCHIVE
    ARCHIVE --> BIGOBJ["LogEntryArchive__b"]
    RETENTION --> RETDATE["Log__c retention date"]
    SLACK --> MSG["Slack message payload"]
    SLACK -.->|"async via Queueable"| QUEUE["execute(System.QueueableContext)"]
```
