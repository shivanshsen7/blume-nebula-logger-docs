...
```mermaid
flowchart TD
    subgraph Capture["Trigger Handlers"]
        EVT["LogEntryEvent__e"] --> HANDLERS["LoggerSObjectHandler subclasses<br/>LogHandler, LogEntryHandler, LogEntryTagHandler,<br/>LoggerTagHandler, LoggerScenarioHandler, LogEntryEventHandler"]
        HANDLERS --> RECORDS["Log__c / LogEntry__c / related records"]
    end
    RECORDS -->|"LogRetentionDate__c reached"| PURGER
    subgraph Purge["Batch / Scheduled Purge"]
        SCHED["LogBatchPurgeScheduler"] --> PURGER["LogBatchPurger"]
        PURGER -->|"chains: LogEntryTag__c to LogEntry__c to Log__c"| DELETED["records deleted"]
    end
    CTRL["LogBatchPurgeController"] -.->|"manual run"| PURGER
    SEL["LogManagementDataSelector"] -.->|"queries"| RECORDS
    LWC["LWC Controllers<br/>LogViewerController, RelatedLogEntriesController, etc."] -.->|"uses"| SEL
```
