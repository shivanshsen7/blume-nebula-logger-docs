...
<Tabs>
<Tab title="Apex">
Use the `setField()` instance methods on `LogEntryEventBuilder` to populate custom fields:
...
Use the `setField()` function in `logEntryBuilder.js`, passing an object containing your custom field names and values:
