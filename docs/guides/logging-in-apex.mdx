---
title: Logging in Apex
description: How Apex developers call Logger, control saving behavior, relate async job logs, and use the fluent interface and LogMessage.
sidebar:
  label: Logging in Apex
  order: 5
---
...
| `SaveMethod` value | Behavior |
| --- | --- |
| `EVENT_BUS` | The default save method. Uses the `EventBus` class to publish `LogEntryEvent__e` records. The default can also be controlled declaratively via `LoggerSettings__c.DefaultSaveMethod__c`. |
| `QUEUEABLE` | Triggers `Logger` to save any pending records asynchronously using a queueable job. Useful when you need to defer some CPU usage and other limits consumed by Logger. |
| `REST` | Uses the current user's session ID to make a synchronous callout to the org's REST API. Useful when you have other callouts being made and need to avoid mixed DML operations. |
| `SYNCHRONOUS_DML` | Skips publishing `LogEntryEvent__e` platform events and instead immediately creates `Log__c` and `LogEntry__c` records. Useful when logging from within the context of another platform event and/or you don't anticipate exceptions in the current transaction. |

<Panel>
When using `SYNCHRONOUS_DML`, any exceptions will prevent your log entries from being saved - Salesforce will roll back any DML statements, including your log entries. Use this save method cautiously.
</Panel>

### Track related logs in batchable and queueable jobs

In Salesforce, asynchronous jobs like batchable and queueable run in separate transactions - each with its own unique transaction ID. To relate these jobs back to the original log, Apex developers can use the method `Logger.setParentLogTransactionId(String)`. `Logger` uses this value to relate child `Log__c` records, using the field `Log__c.ParentLog__c`. The examples below relate all of a batch or queueable job's logs back to the transaction that kicked it off.

<Tabs>
  <Tab title="Batchable">

```apex
// Deploy this class, then run it with: Database.executeBatch(new BatchableLoggerExample());
...
```apex
// Queueable jobs can chain the same way — each instance passes parentLogTransactionId to the next.
// Deploy this class, then run it with: System.enqueueJob(new QueueableLoggerExample(3));
...
sequenceDiagram
    ...
    participant Exec as execute() (per batch scope)
    participant Finish as finish()

    Start->>Start: originalTransactionId = Logger.getTransactionId()
    Start->>Start: log + save (parent Log__c)
    Exec->>Exec: Logger.setParentLogTransactionId(originalTransactionId)
    Exec->>Exec: log each record + save (child Log__c)
    Finish->>Finish: Logger.setParentLogTransactionId(originalTransactionId)
    Finish->>Finish: log completion + save (child Log__c)
    Note over Start,Finish: each phase runs in its own transaction —<br/>ParentLog__c links every child Log__c back to the original
```
...
Each of the logging methods in `Logger` returns an instance of the class `LogEntryEventBuilder`. This class provides several additional methods that can be chained together to further customize each log entry. In the example below, 3 log entries are created using different approaches for calling `Logger` - all 3 approaches result in identical log entries.
...
*Adapted from the [Nebula Logger wiki](https://github.com/jongpie/NebulaLogger/wiki/Logging-in-Apex), © Jonathan Gillespie and contributors, MIT License.*
